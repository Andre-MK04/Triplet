...
def _patch_jobs(monkeypatch, calls):
    ...
    monkeypatch.setattr(tick, "refresh_featured_deals", lambda: calls.append("featured") or {"ok": True})
    ...
    monkeypatch.setattr(
        ...
        lambda: type("Provider", (), {"delivers": True})(),
    )
...
def test_tick_runs_deals_and_alerts_every_hour_but_retention_only_at_configured_hour(monkeypatch):
    ...
    assert calls == ["deals", "featured", "alerts"]  # no retention at 10:00
    assert off_hour["alertsRun"] == 2 and off_hour["retention"] is None

    calls.clear()
    _patch_jobs(monkeypatch, calls)
    on_hour = tick.run_tick(now=datetime(2026, 8, 1, 3, 0))
    assert calls == ["deals", "featured", "alerts", "retention"]  # retention fires at 03:00
