        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if draft.baseLocationId != nil {
                Label("Base set: \(draft.homeLocation)", systemImage: "checkmark.circle.fill")
                    .font(FarelinTypography.font(.footnote, weight: .medium))
        ...
        VStack(spacing: 12) {
            ForEach(Self.comfortRules, id: \.ruleID) { rule in
                VStack(alignment: .leading, spacing: 10) {
                    Text(rule.title).font(FarelinTypography.font(.headline))
                    Text(rule.detail).font(FarelinTypography.font(.caption)).foregroundStyle(.secondary)
                    Picker(
                        rule.title,
                        selection: Binding(
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 13) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(title).font(FarelinTypography.font(.headline)).foregroundStyle(.primary)
                    if let detail {
                        Text(detail).font(FarelinTypography.font(.caption)).foregroundStyle(.secondary).multilineTextAlignment(.leading)
                    }
                }
                Spacer()
            }
        ...
        Button(action: action) {
            HStack(spacing: 13) {
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        Text(airport.city ?? airport.name).font(FarelinTypography.font(.headline))
                        Text(airport.iataCode).font(FarelinTypography.font(.subheadline, weight: .semibold, family: .mono))
                    }
                    Text(airportDetail).font(FarelinTypography.font(.caption)).foregroundStyle(.secondary).lineLimit(2)
                }
                Spacer()
    ...
    var body: some View {
        VStack(spacing: 10) {
            HStack {
                ...
                Spacer()
                Text("\(value) days").font(FarelinTypography.font(.headline).monospacedDigit())
            }
            Slider(
