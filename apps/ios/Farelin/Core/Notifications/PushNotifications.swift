...
struct PushPreferencesSection: View {
    ...
    var body: some View {
        Section("Notifications") {
            ...
            } else { Text("Push delivery is not configured for this build. Your email watch settings still apply.").font(FarelinTypography.font(.footnote)).foregroundStyle(.secondary) }
            ...
            if let message = notifications.message { Text(message).font(FarelinTypography.font(.footnote)).foregroundStyle(.secondary) }
        }
